...
$f.addPlugin("ipad", function (options) {
    ...
    function addAPI() {
        ...
        function fixClip(clip) {
            ...
            var getPathfromUrl = extendedClip.ipadUrl.split("?")[0];
            console.log(getPathfromUrl);
            extendedClip.extension = getPathfromUrl.substr(getPathfromUrl.lastIndexOf('.'));
    ...
    if (isiDevice || opts.simulateiDevice) {
        if (!window.flashembed.__replaced) {
            ...
            window.flashembed = function (root, opts, conf) {
                ...
                api.style.webkitTransformStyle = 'preserve-3d';
                api.style.webkitTransform = 'translateZ(0)';
                api.style.zIndex = -10;
