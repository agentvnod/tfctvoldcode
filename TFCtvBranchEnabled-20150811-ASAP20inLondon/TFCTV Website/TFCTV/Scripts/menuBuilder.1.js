...
function getFeatureEpisodeDetails(showid) {
    ...
    jQuery.get("/Menu/GetPreviewEpisodes/" + showid, function (list) {
        ...
        $.each(list, function (index) {
            pass += '<div class="menuEpisodeFeature"><a href="#"><img src="' + this.EpisodeImageUrl + '"></a><p>Date Aired:' + this.EpisodeAirDate + '</p><p>' + this.Blurb + '</p><p><a href="#"><img src="../../../Content/images/menuBar/WatchNow.jpg"></a></p></div>';
...
var revmenu = function (params) {
    ...
    this.create = function () {
        ...
        $.each(params.data, function (x, item) {
            html += '<li id="source' + this.container + '" class="liAllMenu"></li>';
    ...
    jQuery.get(url, function (data) {
        ...
        $.each(data, function () {
            ...
            $.each(this.shows, function (x, y) {
                ...
                //                else {
                //                    var c = this.Blurb;
                //                    html += '<h3>LIVE STREAMING</h3>';
                //                    html += '<div class="dsourceEpisodes dsourceMovieEpisode"><div class="posterImage"><a href="/Episode/Details/' + this.id + '"><img src="' + this.image + '"></a></div><div class="blurbWatchNow"><p>';
                //                    if (c && c.length > 360)
                //                        html += Ellipsis(c, 400)
                //                    else
                //                        html += this.blurb;
                //                    html += '</p><p><a class="watchnow watchnowmovie" href="/Episode/Details/' + this.id + '"></a></p></div></div>';
                //                }
                shtml += '</div>';
                ...
                if (id == "menuEntertainment" || id == "menuNews")
                    shtml += '<div class="moreLink"><a href="/Show/Details/' + this.id + '" class="more">All Episodes>></a></div>';
                shtml += '</div></li>';
            });

            shtml += '<div class="moreLink"><a href="/Category/List/' + this.id + '" class="more">All ' + this.name + ' Shows>></a></div>';
            shtml += '</ul></div></li>';
        });
        shtml += '</ul></div>';
