    ...
    this.topUsers = function () {
        ...
        $.each(params.json_str, function (x, y) {
            ...
            if (params.type == 'profile') {
                ...
                html += '<span><span class="tp-points">' + y.achievements[0].pointsTotal + '</span> Points</span></div>';
            }
            html += '</li>';
            if (ctr == params.itemperslide) {
                html += '</ul>'; ctr = 0; //reset counter
                html += '<div class="clear" style="border-bottom: 1px solid #393939; width: 920px;"></div>';
            }
            ctr++;
            ctor++;
            if (ctor > params.maxCount)
        ...
        if (json_length == 0)
            $('#' + params.container + ' .itemListBody .wrapper').html('<div class="breakStandard"></div><div class="white padLeftLarge">No data available.</div>');
