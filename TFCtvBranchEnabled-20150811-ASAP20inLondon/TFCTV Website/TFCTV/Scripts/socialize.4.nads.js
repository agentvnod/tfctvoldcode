...
// Get Reaction Count
tfc.Socialize.GetReactionDBCount = function (params) {
    var p = params.barID.split('_');
    var objectID = p[1];
    var objectType = p[0].toLowerCase();

    $.ajax({
        type: "GET",
        url: '/SocialEngagement/GetLovedCount/?ID=' + objectID + '&type=' + objectType,
        context: document.body
    }).done(function (data) {
        $('#' + params.lcontainer).html(data);
        $('#' + params.lcontainer).show();
    });
}

tfc.Socialize.LoveDBCountWithTemplate = function (params) {
    var obj = {};
        ...
        gigya.services.socialize.showReactionsBarUI(
            {
                ...
                cid: params.cid,
                shortURLs: 'never',
                buttonTemplate: '<div><div class="limage" style="float: left;"><img id="LoveImg" onclick="$onClick" src="$iconImg" /></div><div style="background-color:#d7d7d7;width: 40px; height: 24px; float: left; position: relative; top: 8px; text-align: center;"><div class="lcount1" id="lcount1" style="position: relative; top: 4px; font-size: 14px !important;">$count</div></div>',
                onReactionClicked: obj.onReactionClicked,
                onReactionUnclicked: obj.onReactionUnclicked,
                onSendDone: obj.onSendDone,
                onLoad: obj.onLoad
