var carousel = function (params) {
    this.listing = function () {
        ...
        $.each(params.json_str, function (x, y) {
            ...
            html += '<a href="' + y.TargetUrl + '">';
            html += '<img width="940" height="300" src="' + y.BannerImageUrl + '" />';
            html += '</a><div class="bannerInfo">';
            html += '<div class="bInfo">';
            html += '<span class="title"><a href="' + y.TargetUrl + '">' + y.Header + '</a></span>';
            html += '<p>' + y.Blurb + '</p>';
            html += '<div class="button"><span class="cbtn"><a href="' + y.TargetUrl + '">' + y.ButtonLabel + '</a></span></div>';
            html += '</div></div></div>';
            return (x < ctr);
        });
        ...
        if (params.json_str.length > 0) {
            ...
            $('#' + params.container + ' .featureBannerItems .wrapper').cycle({ fx: 'scrollHorz', speed: 1000,
                timeout: 5000, next: '#' + params.container + ' .navigation .next', prev: '#' + params.container + ' .navigation .prev', pause: 1
